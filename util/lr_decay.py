...
def param_groups_lrd(model, weight_decay=0.05, no_weight_decay_list=[], layer_decay=.75):
    ...
    elif hasattr(model.DinoEncoder, 'blocks'):
        num_layers = len(model.DinoEncoder.blocks) + 1
    elif hasattr(model, 'stages'):
        # for convnext
        num_layers = len(model.stages[0])+len(model.stages[1])+len(model.stages[2])+len(model.stages[3])+1
    else:
