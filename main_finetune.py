...
from util.loss import init_loss
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='ConvNeXtV2_large', type=str, metavar='MODEL',
    ...
    parser.add_argument('--lrd_total', type=float, default=0.2,
                        help='A total lr decay for params except for the head')
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='../APTOS2019/', type=str,
                        help='dataset path')
    parser.add_argument('--nb_classes', default=3, type=int,
                        help='number of the classification types')
    parser.add_argument('--cls_task', default=3, type=int,
                        help='The classification task (5 (all), 3, and 2 (binary) )')
    ...
    parser.add_argument('--loss_func', default='CE',type = str, help='loss functions in CE,WCE,FOCAL')
...
def main(args, criterion):
    ...
    if args.model=='RETFound_mae':
        ...
        require_init = False
    elif  args.model=='DinoV2_Vanliia':
        model = timm.create_model('dinov2_vanliia',
                                  num_classes = args.nb_classes,
                                  drop_path_rate=args.drop_path)
        require_init = False
    else:
    ...
    if require_init and args.finetune and not args.eval:
    ...
    # 定义分类头参数的关键词（根据模型结构调整）
    # HEAD_KEYWORDS = {'head', 'fc', 'classifier', 'cls'}
    #
    # # 参数分组逻辑
    # no_weight_decay = model_without_ddp.no_weight_decay() if hasattr(model_without_ddp, 'no_weight_decay') else []
    #
    # param_groups = [
    #     {"params": [], "lr": args.lr * 0.1},  # Backbone组
    #     {"params": [], "lr": args.lr}  # Head组
    # ]
    #
    ...
    #
    #     # 确定权重衰减
    #     wd = 0.0 if (name in no_weight_decay or param.ndim == 1) else args.weight_decay
    #
    #     # 添加到对应组别
    #     group_idx = 1 if is_head else 0
    #     param_groups[group_idx]["params"].append(param)
    #     param_groups[group_idx].setdefault("weight_decay", wd)  # 保持组内统一WD
    #
    ...
    for name, param in model.named_parameters():
        ...
        is_head = any(kw in name for kw in HEAD_KEYWORDS)
        no_wd = name in no_weight_decay or param.ndim == 1

        group = {
            'params': [param],
            'lr': args.lr if is_head else args.lr *args.lrd_total,
            'weight_decay': 0.0 if no_wd else args.weight_decay
        }
    ...
    optimizer_groups = head_groups + backbone_groups

    merged_groups = {}
    for group in optimizer_groups:
        key = (group['lr'], group['weight_decay'])
        if key not in merged_groups:
            merged_groups[key] = group
        else:
            merged_groups[key]['params'].extend(group['params'])

    optimizer = torch.optim.AdamW(list(merged_groups.values()))
