...
def train_one_epoch(
    ...
    metric_logger.add_meter('base_lr', misc.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('head_lr', misc.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, f'Epoch: [{epoch}]')):
        ...
        metric_logger.update(base_lr=base_lr,head_lr=head_lr)
        ...
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            ...
            log_writer.add_scalar('base_lr', base_lr, epoch_1000x)
            log_writer.add_scalar('head_lr', head_lr, epoch_1000x)
...
def evaluate(data_loader, model, device, args, epoch, mode, num_class, log_writer):
    ...
    cm = confusion_matrix(true_labels, pred_labels, labels=list(range(num_class)))
    score = (f1 + roc_auc + kappa) / 3
    # calculate the specificities
    specificities = []
    global_tn, global_fp = 0, 0
    ...
    micro_specificity = global_tn / (global_tn + global_fp + 1e-7)


    if log_writer:
        for metric_name, value in zip(['accuracy', 'f1', 'roc_auc', 'hamming', 'jaccard', 'precision', 'recall', 'average_precision', 'kappa', 'score', 'micro_specificity', 'macro_specificity'],
                                       [accuracy, f1, roc_auc, hamming, jaccard, precision, recall, average_precision, kappa, score, micro_specificity, macro_specificity]):
    ...
    print(f'Accuracy: {accuracy:.4f}, F1 Score: {f1:.4f}, ROC AUC: {roc_auc:.4f}, Hamming Loss: {hamming:.4f},\n'
          ...
          f' Macro Specificity: {macro_specificity:.4f}, Micro Specificity: {micro_specificity:.4f},\n')
    ...
    with open(results_path, 'a', newline='', encoding='utf8') as cfa:
        wf = csv.writer(cfa)
        if not file_exists:
            wf.writerow(['val_loss', 'accuracy', 'f1', 'roc_auc', 'hamming', 'jaccard', 'precision', 'recall', 'average_precision', 'kappa','micro_specificity', 'macro_specificity'])
        wf.writerow([metric_logger.meters["loss"].global_avg, accuracy, f1, roc_auc, hamming, jaccard, precision, recall, average_precision, kappa,micro_specificity, macro_specificity])
